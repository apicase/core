...
const apicase = adapter => req => {
  ...
  if (
    ...
    req.options.delay ||
    req.options.timeout ||
    req.options.debounce ||
    !req.options.immediate
  ) {
    console.warn(
      `[Apicase.deprecated] Delayed requests and features of Requests+ are now deprecated. Use https://github.com/apicase/spawner instead`
    )
  }
