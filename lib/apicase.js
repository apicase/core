...
const compose = (hooks, createData) => (payload, changePayload) =>
  new Promise(resolve => {
    ...
    const next = payload => {
      ...
      return compose(hooks.slice(1), createData)(payload, changePayload).then(
        resolve
      )
    }
  ...
  const doRequest = function(payload) {
    return new Promise(resolve => {
      const resolveCb = result =>
        composedHooks.resolve(result).then(() => resolve(res.state))

      const rejectCb = result =>
        composedHooks.reject(result).then(() => resolve(res.state))
