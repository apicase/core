...
const apicase = adapter => (req, opts) => {
  ...
  const start = payload => {
    ...
    res.promise = composeHooks(before)(payload)
  ...
  const doRequest = function(payload) {
    return new Promise(resolve => {
      const doneCb = result => {
        setResult(result)
        composeHooks(done)(result).then(() => resolve(pickState(res)))
      }

      const failCb = result => {
        setResult(result)
        composeHooks(fail)(result).then(() => resolve(pickState(res)))
      }
  ...
  const [before, done, fail] = [
    {
      ...
      handleError: ctx => bus.emit('error', ctx),
      update(diff) {
        if (process.env.NODE_ENV !== 'production') {
